...
medals = ['Eisernes Kreuz','Deutschen Kreuz',
'Ritterkreuz','Dienstauszeichnung','Ritterkreuz des Eisernen Kreuzes mit Eichenlaub']
active_awardees = [0,0,0,0,0,0,0]
dead_awardees = [0,0,0,0,0,0,0]
alumni_awardees = [0,0,0,0,0,0,0]
for i,medal in enumerate(medals):
    for g in active_roster:
        if medal in g.awards: active_awardees[i]+=1
    for g in dead_roster:
        if medal in g.awards: dead_awardees[i]+=1
    for g in alumni_roster:
        if medal in g.awards: alumni_awardees[i]+=1
...
for i,rank in enumerate(ranks):
    for g in active_roster:
        if rank in g.rank: active_ranks[i]+=1
    for g in dead_roster:
        if rank in g.rank: dead_ranks[i]+=1
    for g in alumni_roster:
        if rank in g.rank: alumni_ranks[i]+=1
...
print('Company turn over rate (%)',float(len(dead_roster+alumni_roster))/len(active_roster)*100.0)
